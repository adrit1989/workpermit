...
// --- FIX 1: CUSTOM DATE PARSER (Prevents 500 Crash) ---
function parseInputDate(dateStr) {
    ...
    if (typeof dateStr === 'string' && dateStr.indexOf('-') === 2) {
        try {
            ...
            // Convert to ISO: YYYY-MM-DDTHH:mm:00
            return new Date(`${dateParts[2]}-${dateParts[1]}-${dateParts[0]}T${timePart}:00`);
        } catch (e) {
            return new Date(); // Safety fallback
        }
    }
...
// --- FIX 2: SAVE PERMIT WITH DATE PARSER & SAFETY FLAGS ---
...
app.post('/api/update-status', async (req, res) => {
    try {
        ...
        // FIX 3: CAPTURE SITE RESTORED CHECK
...
app.post('/api/renewal', async (req, res) => {
    try {
        ...
        if (userRole === 'Requester') {
            // FIX: PARSE DATES HERE TOO
            const rs = parseInputDate(data.RenewalValidFrom);
            const re = parseInputDate(data.RenewalValidTo);
            const pS = new Date(cur.recordset[0].ValidFrom);
            const pE = new Date(cur.recordset[0].ValidTo);
...
app.get('/api/download-pdf/:id', async (req, res) => {
    try {
        ...
        // FIX 4: SIGNATURE PAGE BREAK LOGIC
        if (doc.y > 650) { doc.addPage(); drawHeaderOnAll(); doc.y = 135; }
