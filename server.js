...
// <--- FIX 2: REMOVED authenticateToken from here so login page can load users
...
app.post('/api/save-permit', authenticateToken, upload.any(), async (req, res) => {
    try {
        ...
        const q = pool.request()
            ...
            .input('j', sql.NVarChar(sql.MAX), JSON.stringify(permitData)) // Use permitData
...
app.post('/api/renewal', authenticateToken, upload.any(), async (req, res) => {
    try {
        // RENAMED VARIABLE TO 'renewalFields' (Fixes SyntaxError)
        const { PermitID, action, rejectionReason, renewalWorkers, oddHourReq, ...renewalFields } = req.body;
        ...
        if (userRole === 'Requester') {
            const rs = new Date(renewalFields.RenewalValidFrom);
            const re = new Date(renewalFields.RenewalValidTo);
            ...
            r.push({
                status: 'pending_review',
                valid_from: renewalFields.RenewalValidFrom, valid_till: renewalFields.RenewalValidTo,
                hc: renewalFields.hc, toxic: renewalFields.toxic, oxygen: renewalFields.oxygen,
                precautions: renewalFields.precautions,
