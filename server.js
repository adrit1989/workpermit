...
// Helper: Consistent Date Formatting (DD-MM-YYYY HH:mm)
function getNowIST() { 
...
function formatDate(dateStr) {
    ...
    if (isNaN(d.getTime())) return dateStr; // Return as-is if strictly string
...
app.post('/api/save-permit', upload.single('file'), async (req, res) => {
    try {
        ...
        await pool.request().input('pid', sql.NVarChar, newId).input('status', sql.NVarChar, 'Pending Review').input('wt', sql.NVarChar, req.body.WorkType).input('req', sql.NVarChar, req.body.RequesterEmail).input('rev', sql.NVarChar, req.body.ReviewerEmail).input('app', sql.NVarChar, req.body.ApproverEmail).input('vf', sql.DateTime, vf).input('vt', sql.DateTime, vt).input('lat', sql.NVarChar, req.body.Latitude || null).input('lng', sql.NVarChar, req.body.Longitude || null).input('locSno', sql.NVarChar, req.body.LocationPermitSno).input('iso', sql.NVarChar, req.body.RefIsolationCert).input('cross', sql.NVarChar, req.body.CrossRefPermits).input('jsa', sql.NVarChar, req.body.JsaRef).input('mocReq', sql.NVarChar, req.body.MocRequired).input('mocRef', sql.NVarChar, req.body.MocRef).input('cctv', sql.NVarChar, req.body.CctvAvailable).input('cctvDet', sql.NVarChar, req.body.CctvDetail).input('vendor', sql.NVarChar, req.body.Vendor).input('dept', sql.NVarChar, req.body.IssuedToDept).input('locUnit', sql.NVarChar, req.body.LocationUnit).input('exactLoc', sql.NVarChar, req.body.ExactLocation).input('desc', sql.NVarChar, req.body.Desc).input('offName', sql.NVarChar, req.body.OfficialName).input('json', sql.NVarChar, JSON.stringify(fullData))
...
app.post('/api/update-status', async (req, res) => {
    try {
        ...
        let status = p.Status;
        const now = getNowIST();
        const sig = `${user} on ${now}`; // Saves "Name on Date"
        ...
        if (role === 'Reviewer') {
            ...
            else if (action === 'reject_closure') { status = 'Active'; data.Closure_Reviewer_Display = `Rejected by ${user} on ${now}: ${comment}`; }
        }
            ...
            else if (action === 'reject_closure') { status = 'Active'; data.Closure_Issuer_Remarks = `Closure Rejected by ${user} on ${now}: ${comment}`; }
        }
        else if (role === 'Requester' && action === 'initiate_closure') {
            status = 'Closure Pending Review'; data.Closure_Receiver_Sig = sig; data.Site_Restored_Check = "Yes"; 
        }
    ...
    try {
        ...
        if (userRole === 'Requester') {
             ...
             if (renewals.length > 0) {
                 ...
                 if (reqStart < lastEnd && renewals[renewals.length - 1].status !== 'rejected') return res.status(400).json({ error: "Renewal must start after the previous renewal ends." });
             }
...
app.post('/api/map-data', async (req, res) => {
    try {
        ...
        res.json(result.recordset.map(row => {
            ...
            return { PermitID: row.PermitID, lat: parseFloat(row.Latitude), lng: parseFloat(row.Longitude), WorkType: d.WorkType, Desc: d.Desc, ExactLocation: d.ExactLocation };
        }));
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/download-excel', async (req, res) => {
    try {
        ...
        worksheet.columns = [{ header: 'Permit ID', key: 'id' }, { header: 'Status', key: 'status' }, { header: 'Work Type', key: 'wt' }, { header: 'Requester', key: 'req' }, { header: 'Department', key: 'dept' }, { header: 'Vendor', key: 'vendor' }, { header: 'Description', key: 'desc' }, { header: 'Location', key: 'loc' }, { header: 'Valid From', key: 'vf' }, { header: 'Valid To', key: 'vt' }];
...
// 12. PDF DOWNLOAD (Format Fixed: Names & Dates Separated)
app.get('/api/download-pdf/:id', async (req, res) => {
    try {
        ...
        // Helper to Split "Name on Date" string
        ...
        const otherRows = [[`Vendor: ${d.Vendor||'-'}`, `Issued To: ${d.IssuedToDept}`], [`Isolation Cert: ${d.RefIsolationCert||'-'}`, `Cross Ref: ${d.CrossRefPermits||'-'}`], [`JSA Ref: ${d.JsaRef||'-'}`, `MOC Ref: ${d.MocRef||'NA'}`], [`CCTV Avail: ${d.CctvAvailable}`, `CCTV Detail: ${d.CctvDetail||'-'}`]];
        ...
        const gpRows = gpQs.map((q, i) => { let ans = d[q.id] === 'Yes' ? 'YES' : 'NA'; if(q.id === "GP_Q12") ans = `Tox:${d.GP_Q12_ToxicGas||'-'} HC:${d.GP_Q12_HC||'-'} O2:${d.GP_Q12_Oxygen||'-'}`; let desc = q.d ? (d[q.d] || '') : ''; return [`${i+1}`, q.t, ans, desc]; });
        ...
        const spRows = spQs.map((q, i) => { let ans = d[q.id] === 'Yes' ? 'YES' : 'NA'; let desc = q.d ? (d[q.d] || '') : ''; return [`${i+1}`, q.t, ans, desc]; });
        ...
        // SIGNATURES - SPLIT NAME AND DATE
        doc.font('Helvetica-Bold').text('DIGITAL SIGNATURES');
        const reqSig = { name: d.RequesterName, date: formatDate(p.ValidFrom) }; // Fallback to start date
        ...
        for (let i = 0; i < range.count; i++) { doc.switchToPage(i); doc.save(); doc.rotate(-45, { origin: [300, 400] }); doc.fontSize(80).fillColor(color).opacity(0.15).text(watermarkText, 100, 350, { align: 'center', width: 400 }); doc.restore(); }
