...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if(r.recordset.length > 0) {
            ...
            res.json({success:true, user:{ Name: u.Name||u.name||u.NAME, Email: u.Email||u.email||u.EMAIL, Role: u.Role||u.role||u.ROLE }});
...
app.get('/api/users', async (req, res) => {
    try {
        ...
        const mapU = (u) => ({ name: u.Name||u.name||u.NAME, email: u.Email||u.email||u.EMAIL, role: u.Role||u.role||u.ROLE });
...
app.post('/api/save-permit', upload.single('file'), async (req, res) => {
    try {
        ...
        q.input('p', sql.NVarChar, pid).input('s', sql.NVarChar, 'Pending Review').input('w', sql.NVarChar, req.body.WorkType)
...
// 7. RENEWALS (8 Hr Limit, Sequential)
app.post('/api/renewal', async (req, res) => { try { 
    const { PermitID, userRole, userName, action, rejectionReason, ...data } = req.body;
    const pool = await getConnection(); 
    const cur = await pool.request().input('p', sql.NVarChar, PermitID).query("SELECT RenewalsJSON, Status, ValidFrom, ValidTo FROM Permits WHERE PermitID=@p"); 
    ...
    if(userRole==='Requester') {
        ...
        if(rs<ps || re>pe) return res.status(400).json({error:"Out of Permit Validity"});
    ...
    const newStatus = r[r.length-1].status==='approved'?'Active':(r[r.length-1].status==='rejected'?'Active':(userRole==='Requester'?'Renewal Pending Review':'Renewal Pending Approval'));
...
// 8. PDF GENERATOR (EXACT OISD-105 FORMAT)
app.get('/api/download-pdf/:id', async (req, res) => {
    try {
        ...
        const drawHeader = () => {
             ...
             doc.rect(startX, startY, 80, 95).stroke(); // Logo
             doc.rect(startX + 80, startY, 320, 95).stroke(); // Title
             doc.rect(startX + 400, startY, 135, 95).stroke(); // Doc Control
             doc.font('Helvetica-Bold').fontSize(12).text('INDIAN OIL CORPORATION LIMITED', startX+80, startY+15, {width:320, align:'center'});
             doc.fontSize(10).text('EASTERN REGION PIPELINES', startX+80, startY+30, {width:320, align:'center'});
             doc.text('HSE DEPT.', startX+80, startY+45, {width:320, align:'center'});
             doc.fontSize(9).text('COMPOSITE WORK PERMIT (OISD-105)', startX+80, startY+65, {width:320, align:'center'});
             doc.fontSize(8).font('Helvetica'); doc.text('Doc No: ERPL/HS&E/25-26', startX+405, startY+20); doc.text('Date: 01.09.2025', startX+405, startY+50);
        };

        // Page 1
        drawHeader(); doc.y = 135; doc.fontSize(9).font('Helvetica');
        const infoY = doc.y; const col1 = 40, col2 = 300;
        doc.text(`Permit No: ${p.PermitID}`, col1, infoY); doc.text(`Validity: ${formatDate(p.ValidFrom)} - ${formatDate(p.ValidTo)}`, col2, infoY);
        doc.text(`Issued To: ${d.IssuedToDept} (${d.Vendor})`, col1, infoY+15); doc.text(`Location: ${d.ExactLocation}`, col2, infoY+15);
        doc.text(`Desc: ${d.Desc}`, col1, infoY+30, {width:500});
        doc.text(`Site Person: ${d.RequesterName}`, col1, infoY+60); doc.text(`Security: ${d.SecurityGuard||'-'}`, col2, infoY+60);
        doc.text(`Emergency: ${d.EmergencyContact||'-'}`, col1, infoY+75); doc.text(`Fire Stn: ${d.FireStation||'-'}`, col2, infoY+75);
        ...
        doc.font('Helvetica-Bold').fontSize(10).text("HAZARDS & PRECAUTIONS", 30, doc.y); doc.y += 15;
        doc.rect(30, doc.y, 535, 60).stroke(); doc.fontSize(8).font('Helvetica');
        ...
        doc.font('Helvetica').fontSize(8);
        renewals.forEach(r => { doc.rect(30, doc.y, 100, 30).stroke().text(`${formatDate(r.valid_from)}\n${formatDate(r.valid_till)}`, 35, doc.y+5); doc.rect(130, doc.y, 150, 30).stroke().text(`HC:${r.hc} Tox:${r.toxic} O2:${r.oxygen}\nPrec:${r.precautions||'-'}`, 135, doc.y+5); doc.rect(280, doc.y, 285, 30).stroke().text(`${r.req_name} | ${r.rev_name||'-'} | ${r.app_name||'-'}`, 285, doc.y+10); doc.y += 30; }); doc.y += 20;

        // Closure
        doc.font('Helvetica-Bold').fontSize(10).text("CLOSURE", 30, doc.y); doc.y += 15; doc.rect(30, doc.y, 535, 70).stroke(); doc.fontSize(8);
        doc.text("1. RECEIVER: Work stopped & area cleared.", 35, doc.y + 10); doc.font('Helvetica').text(`${d.Closure_Requestor_Remarks || '-'} (${d.Closure_Requestor_Date || ''})`, 300, doc.y + 10);
        ...
        doc.addPage(); drawHeader(); doc.y = 135; doc.font('Helvetica-Bold').fontSize(10).text("GENERAL INSTRUCTIONS", 30, doc.y); doc.y += 15; doc.font('Helvetica').fontSize(8);
